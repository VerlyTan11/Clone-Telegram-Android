...
export default function CustomDrawer(props) {
  ...
  return (
    <DrawerContentScrollView contentContainerStyle={{ flex: 1 }}>
      <View className={`${isDarkMode ? 'bg-darken' : 'bg-default-blue'} p-4`}>
        <View className="flex-col">
          <View className="flex-row justify-between items-center">
            ...
            <Pressable onPress={toggleTheme}>
              <Image source={isDarkMode ? require('../assets/sun.png') : require('../assets/moon.png')} className="w-5 h-5" />
            </Pressable>
          </View>
      ...
      <View className={`${isDarkMode ? 'bg-light-dark text-white' : 'bg-white'} flex-1`}>
        {isExpanded && (
          <View className="mt-4 p-2 bg-white rounded-lg">
            ...
            >
              ...
              <Text className={`text-sm ${isDarkMode ? 'text-white' : 'text-darken'} pl-8`}>Tambah Akun</Text>
            </Pressable>
